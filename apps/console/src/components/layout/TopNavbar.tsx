  ...
  return (
    <header className="h-16 bg-slate-900 border-b border-slate-800 flex items-center justify-between px-6 z-30 shrink-0">
      ...
      <div className="flex items-center gap-4">
        ...
        <div
          className={`hidden lg:flex items-center gap-2 px-3 py-1.5 rounded-full border text-xs font-semibold ${
            rustDeskConfig.onlineState
              ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400'
              : 'bg-rose-500/10 border-rose-500/20 text-rose-400'
          }`}
        >
          <Radio className={`w-3.5 h-3.5 ${rustDeskConfig.onlineState ? 'animate-pulse' : ''}`} />
          <span>RustDesk Relay: {rustDeskConfig.onlineState ? 'Online' : 'Offline'}</span>
        </div>
      </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden sm:block text-left text-xs">
            <div className="font-bold text-slate-100 flex items-center gap-1.5">
              <span>{currentUser?.name || 'Alex Rivera'}</span>
              <UserCheck className="w-3.5 h-3.5 text-emerald-400" />
            </div>
            <div className="text-[10px] text-slate-400 font-medium capitalize">{currentUser?.role || 'Lead MSP Architect'}</div>
          </div>
        </div>
      </div>
    </header>
